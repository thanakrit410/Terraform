...
resource "aws_sns_topic" "sns_topics" {
  ...
  name = "${each.value.project_name}_LOGICAL_DISK01_FREE_SPACE_${each.value.alarm_type}"
...
resource "aws_cloudwatch_metric_alarm" "metric_alarms" {
  ...
  alarm_name          = "${each.value.project_name}_LOGICAL_DISK01_FREE_SPACE_WARNING"
...
resource "aws_cloudwatch_metric_alarm" "metric_alarms_critical" {
  ...
  alarm_name          = "${each.value.project_name}_LOGICAL_DISK01_FREE_SPACE_CRITICAL"
  comparison_operator = "LessThanOrEqualToThreshold"
  evaluation_periods  = 3
  datapoints_to_alarm = 2
  metric_name         = "LogicalDisk % Free Space"
