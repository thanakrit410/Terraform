...
resource "aws_sns_topic" "sns_topics" {
  ...
  name = "${each.value.project_name}_ECS_MEMORY_UTILIZATION_${each.value.alarm_type}"
...
resource "aws_cloudwatch_metric_alarm" "metric_alarms" {
  ...
  alarm_name          = "${each.value.project_name}/MEMORY_UTILIZATION_WARNING"
  ...
  alarm_description   = "ECS Memory Utilization >= 75% for 3/5 datapoints."
...
resource "aws_cloudwatch_metric_alarm" "metric_alarms_critical" {
  ...
  alarm_name          = "${each.value.project_name}/MEMORY_UTILIZATION_CRITICAL"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  ...
  threshold           = 90
  actions_enabled     = true
  alarm_description   = "ECS Memory Utilization >= 90% for 2/3 datapoints."
